  ...
  if(window.contactList){ //check if we already have a contact list
   ...
   var contacts = contactList.returnAll();
    ...
    if(contacts.length>0){
      ...
      document.getElementById('show-panel').innerHTML += '<div class="contact-item">ITEM DOES NOT EXIST';
      delete contacts[i].name;
      delete contacts[i].phone;
      delete contacts[i].email;
      }
    }else{
...
document.getElementById('js-show-all').addEventListener('click', function(){
  if(window.contactList){ //check if we already have a contact list
   ...
   var contacts = contactList.returnAll();
    ...
    if(contacts.length>0){
      for(var i = 0;i<contacts.length;i++){
      document.getElementById('show-panel').innerHTML += '<div class="contact-item">Name:'+contacts[i].name+'<br>Phone:'+contacts[i].phone+'<br>Email:'+contacts[i].email+'<button   onclick="go()"> delete</div><hr>';
      }
    }else{
      document.getElementById('show-panel').innerHTML += '<div class="contact-item">You have no contacts. Why not add  a few?</div><hr>';
    }
